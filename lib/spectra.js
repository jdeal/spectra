...
function reportSpec(spec,depth){
  ...
  console.log(indentString(spec.title,indent));
  ...
  if (!spec.isDone){
    console.log(indentString("Spec did not finish. Did you use the spec callback?",nextIndent));
  }
...
function finishSpecs(specs){
  ...
  specs.forEach(function (spec){
    ...
    if (spec.children.length === 0 && spec.good.length === 0 && spec.bad.length === 0){
      spec.isFailed = true;
    }
